...
| `ECHONET_COMMAND_TIMEOUT_MS` | `--echonetCommandTimeoutMs` | ECHONET Liteコマンドの応答待ちの時間をミリ秒で指定します。(デフォルト: 3000) |
| `ECHONET_PROPERTY_REQUEST_RETRY_COUNT` | (なし) | プロパティリクエスト失敗時のリトライ回数を指定します。(デフォルト: 1) |
...
> **注意:** フレンドリーネームを設定すると、デバイスのMQTTトピックやREST APIエンドポイントは元のIDではなくフレンドリーネームを使用します。元のIDでのアクセスはできなくなります。これはエイリアス(別名)ではなく、置き換えです。
